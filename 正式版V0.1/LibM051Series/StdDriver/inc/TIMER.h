...
{
    uint32_t u32ClockSetting = 0;
    uint32_t u32ClockSrc;
    u32ClockSetting = ( SYSCLK->CLKSEL1 >> ( 8 + ( eTimerCH * 4 ) ) ) & 0x7;

    if ( u32ClockSetting == 0 )
        {
            /* Option 0: External Clock */
            u32ClockSrc = __XTAL;
        }

    else if ( u32ClockSetting == 2 )
        {
            /* Option 2: HCLK */
            u32ClockSrc = SystemCoreClock;
        }

    else if ( u32ClockSetting == 5 )
        {
            /* Option 5: 10 kHz */
            u32ClockSrc = 10000;
        }

    else if ( u32ClockSetting == 7 )
        {
            /* Option 7: 22MHz*/
            u32ClockSrc = __IRC22M;
        }
