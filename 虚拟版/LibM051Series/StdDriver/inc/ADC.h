...
#define ADC_ADCR_DIFFEN_SINGLE_END      (0UL<<ADC_ADCR_DIFFEN_Pos)   /*!< Single mode                */
 ...
 */
#define _ADC_SET_DIFFERENTIAL_INPUT_MODE(Mode)           (ADC->ADCR |= ADC_ADCR_DIFFEN_Msk)
 ...
 */
#define _ADC_SET_SINGLE_END_INPUT_MODE(Mode)           (ADC->ADCR = ((ADC->ADCR & ~ADC_ADCR_DIFFEN_Msk)))
...
{
    uint8_t u8AdcClkSrcSel;
    uint32_t u32ClkTbl[4] = {__XTAL, 0, 0, __IRC22M};
    u32ClkTbl[1] = PllClock;
    u8AdcClkSrcSel = ( ( SYSCLK->CLKSEL1 & SYSCLK_CLKSEL1_ADC_S_Msk ) >> SYSCLK_CLKSEL1_ADC_S_Pos );
    return ( ( u32ClkTbl[u8AdcClkSrcSel] ) / ( ( ( SYSCLK->CLKDIV & SYSCLK_CLKDIV_ADC_N_Msk ) >> SYSCLK_CLKDIV_ADC_N_Pos ) + 1 ) / 27 );
}
...
static __INLINE uint32_t ADC_GetConversionRate_M05xxxBN()
{
    ...
    u8AdcClkSrcSel = ( ( SYSCLK->CLKSEL1 & SYSCLK_CLKSEL1_ADC_S_Msk ) >> SYSCLK_CLKSEL1_ADC_S_Pos );
    return ( ( u32ClkTbl[u8AdcClkSrcSel] ) / ( ( ( SYSCLK->CLKDIV & SYSCLK_CLKDIV_ADC_N_Msk ) >> SYSCLK_CLKDIV_ADC_N_Pos ) + 1 ) / 21 );
}
